...
For SQL users, please note that you are supposed 
to write a DELETE statement and not a SELECT one.
For Pandas users, please note that you are supposed to
...
DELETE 
FROM person a
	USING person b -- use this to join two tables
WHERE a.id > b.id --will get the id based on ASC
AND a.email =b.email
